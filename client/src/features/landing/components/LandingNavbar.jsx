    ...
    <>
      ...
      <header className="bg-[#1B3E35]/95 border-b border-white/10 sticky top-0 z-50 backdrop-blur-md shrink-0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-13 flex items-center justify-between gap-4">
              ...
              {categoriesOpen && (
                <div className="absolute top-full left-0 mt-1.5 w-64 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl p-2 z-50 text-xs">
                  <div className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-800">
                    Jump Direct to ERP Workspace
                  </div>
                  <div className="space-y-0.5 mt-1">
                    {[
                      { name: "Herd & Milking Hub", path: "/farm", icon: Tractor },
                      { name: "Supplier Intake Dock", path: "/supplier", icon: Truck },
                      { name: "Counter Touch POS", path: "/pos", icon: ShoppingCart },
                      { name: "Customer Khata Ledgers", path: "/customer-khata-ledger", icon: Wallet },
                      { name: "Delivery Fleet Logistics", path: "/delivery", icon: Bike },
                      { name: "Daily Closing & Audit", path: "/finance/daily-closing", icon: Scale },
                    ].map((item) => {
          ...
          {/* Right: User Authentication & CTA Launch Button */}
          <div className="flex items-center space-x-2 shrink-0">
            {isAuthenticated && user ? (
              <div className="flex items-center gap-2">
                <Link
                  to="/dashboard"
                  className="flex items-center gap-1.5 text-xs text-white bg-white/10 hover:bg-white/15 px-2.5 py-1 rounded-full border border-white/15 transition"
                >
                  <span className="w-5 h-5 rounded-full bg-[#00a86b] text-white flex items-center justify-center font-bold text-[9.5px]">
                    {user.avatar || "SA"}
                  </span>
              ...
              <Link
                to="/login"
                className="text-[11px] font-bold text-slate-200 hover:text-white px-2.5 py-1 rounded-full hover:bg-white/10 transition"
              >
                Sign In
              </Link>
            )}
            ...
            <Link
              to="/dashboard"
              className="bg-[#00a86b] hover:bg-[#008f5b] text-white font-bold text-[11px] px-3.5 py-1.5 rounded-full shadow-md shadow-emerald-500/25 transition-all duration-200 flex items-center gap-1 group cursor-pointer"
            >
