      ...
      <div className="bg-white w-full max-w-sm rounded-xl shadow-xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="px-3.5 py-2.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/70">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-xs font-bold text-slate-800">Record Payment</h3>
              <p className="text-[10px] text-slate-400 leading-tight">
                {customer.name} • Due: <span className="font-bold text-slate-700">Rs. {totalDue.toLocaleString()}</span>
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-3 space-y-2.5 text-xs">
              ...
              <div className="flex items-center gap-1 mt-1.5 justify-between">
                ...
                {[25, 50, 75].map((pct) => (
                  <button
                    ...
                    className="px-2 py-0.5 bg-slate-50 hover:bg-emerald-50 hover:border-emerald-300 hover:text-emerald-700 border border-slate-200 rounded text-[10px] font-semibold text-slate-600 transition cursor-pointer"
                  >
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input
                ...
                className="w-full px-2 py-1 border border-slate-200 rounded-md focus:outline-none focus:border-emerald-500 text-slate-800 font-bold text-xs"
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-2 py-1 border border-slate-200 rounded-md focus:outline-none focus:border-emerald-500 text-slate-800 cursor-pointer text-xs"
              >
          ...
          {totalDue > 0 && (
            <div className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-[10px] flex items-center justify-between">
              <div>
                ...
                <span className="font-bold text-slate-700">Rs. {totalDue.toLocaleString()}</span>
              </div>
              <ArrowRight className="w-3 h-3 text-slate-300" />
              <div>
                <span className="text-emerald-600 block font-medium">Paying Now</span>
                <span className="font-bold text-emerald-700">Rs. {enteredAmount.toLocaleString()}</span>
              </div>
              <ArrowRight className="w-3 h-3 text-slate-300" />
              <div>
                <span className="text-slate-400 block">Remaining</span>
                <span className={`font-bold ${remainingBalance === 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
          ...
          <div className="pt-2 flex justify-end gap-1.5 border-t border-slate-100">
            <button
              type="button"
              onClick={onClose}
              className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-md font-semibold transition cursor-pointer text-xs"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-3.5 py-1 bg-[#00a86b] hover:bg-[#00925d] text-white rounded-md font-bold transition shadow-xs cursor-pointer text-xs"
            >
              Record Payment
            </button>
          </div>
