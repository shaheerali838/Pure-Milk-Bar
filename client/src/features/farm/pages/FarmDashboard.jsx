    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {stats.map(({ label, value, sub, icon: Icon, color }) => (
          <div key={label} className="flex items-center gap-3.5 bg-white border border-slate-200 rounded-xl p-4" style={{ borderLeft: "3px solid " + color }}>
            <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0" style={{ background: color + "18" }}>
              <Icon style={{ width: 22, height: 22, color }} />
            </div>
            <div>
              <p className="text-[22px] font-extrabold text-slate-900 leading-none mb-0.5">{value}</p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        <h3 className="text-base font-bold text-slate-800 mb-1">Farm Overview</h3>
