          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-emerald-100 font-mono">
                Invoice: {completedSaleReceipt.invoiceId}
              </p>
            </div>
          </div>
          <button
          ...
          <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-1 text-slate-600 text-[11px]">
            ...
            {(completedSaleReceipt.customer?.phone ||
              (completedSaleReceipt.walkinCustomer?.phone &&
                completedSaleReceipt.walkinCustomer.phone !== "N/A")) && (
              <div className="flex justify-between">
                <span>Phone:</span>
          ...
          <div className="pt-2 border-t border-dashed border-slate-200 space-y-1 text-[11px]">
            <div className="flex justify-between text-slate-500">
              ...
              <span className="tabular">
                Rs. {completedSaleReceipt.subtotal.toLocaleString()}
              </span>
            </div>
            {completedSaleReceipt.deliveryCharge > 0 && (
              <div className="flex justify-between text-slate-500">
                ...
                <span className="tabular">
                  +Rs. {completedSaleReceipt.deliveryCharge.toLocaleString()}
                </span>
              </div>
            )}
            {completedSaleReceipt.discount > 0 && (
              <div className="flex justify-between text-emerald-600 font-semibold">
                <span>Discount:</span>
                <span className="tabular">
                  -Rs. {completedSaleReceipt.discount.toLocaleString()}
                </span>
              </div>
            )}
            <div className="flex justify-between text-sm font-black text-slate-900 pt-1.5 border-t border-slate-200">
              <span>NET PAYABLE:</span>
              <span className="text-purple-700 tabular">
                Rs. {completedSaleReceipt.netPayable.toLocaleString()}
              </span>
            </div>
            ...
            {completedSaleReceipt.cashTendered && (
              <>
                <div className="flex justify-between text-slate-500 pt-1">
                  ...
                  <span className="tabular">
                    Rs. {completedSaleReceipt.cashTendered.toLocaleString()}
                  </span>
                </div>
                {completedSaleReceipt.changeDue > 0 && (
                  <div className="flex justify-between text-emerald-700 font-bold">
                    ...
                    <span className="tabular">
                      Rs. {completedSaleReceipt.changeDue.toLocaleString()}
                    </span>
                  </div>
                )}
              </>
